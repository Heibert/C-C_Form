...
window.addEventListener('load', function (e) {
  ...
  LlamarId("bGuardar").addEventListener("click", (e) => { Promediar(e) })
  botonS.addEventListener("click", (e) => { MostrarS(e) })
  botonA.addEventListener("click", (e) => { MostrarA(e) })
})
//Mostrar la seccion de pregunta
function MostrarS(e) {
  if ((pagina * 5 <= document.querySelectorAll("input:checked").length && document.querySelector("select").value != "") || pagina === 0) {
    pagina++
    let objCreados = 0
    ...
    for (let i = 1; i < 6; i++) {
      document.querySelector(`input[type=radio][value="1"][name="${pagina}-${i}"]`).setAttribute("required", "")
      console.log(document.querySelectorAll(`input[type=radio][value="1"][name="${pagina}-${i}"]`));
    }
    if (pagina > 1) {
